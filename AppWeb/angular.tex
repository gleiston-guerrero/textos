	...
	\section{Angular}
		...
		\subsection{Descripción y Propósito}
			...
			\subsubsection{Configuración del Ambiente de Desarrollo}
				\begin{enumerate}
					\item \textbf{Instalar Node.js LTS} (versión LTS activa o mantenimiento LTS). Angular CLI depende de Node y del gestor de paquetes (npm, pnpm, yarn o bun) \cite{AngularSetupLocal2025}.\\
						\begin{itemize}
							\item \textbf{Node.js LTS (Long-Term Support).}
							\begin{itemize}
								\item \emph{Qué es:} Node.js es el entorno de ejecución de JavaScript en el lado del servidor sobre el que se ejecuta el \emph{tooling} de Angular (incluida la CLI). La variante \textbf{LTS} ofrece estabilidad y correcciones a largo plazo.
								\item \emph{Por qué usar LTS:} Angular publica compatibilidades específicas con versiones de Node; la línea LTS garantiza parches de seguridad y una API estable. En general, se recomienda la \textbf{LTS activa} (novedades + parches) y, si no es posible, la \textbf{LTS en mantenimiento} (parches críticos) antes que versiones “current”.
								\item \emph{Comprobar e instalar:}
								\begin{lstlisting}[language=bash,mathescape=false]
#Instalar en Linux
curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash -
sudo apt-get install -y nodejs
# Este es el método oficial de NodeSource para obtener la versión LTS más reciente de Node.js. Esto instala tanto node como npm.
...
npm -v \end{lstlisting}
								...
								\end{enumerate}
							\end{itemize}
							\item \textbf{Angular CLI y su dependencia de Node.}
							\begin{itemize}
								\item \emph{Qué es:} La \textbf{Angular CLI} (\texttt{ng}) es la herramienta oficial para crear, compilar, probar y desplegar proyectos Angular.
								
								\item \emph{Relación con Node:} La CLI está escrita para el ecosistema Node; necesita una versión de Node compatible para ejecutarse correctamente.
								
								\item \emph{Usar un gestor concreto por proyecto:} puede fijarlo al crear o configurar:
								\begin{lstlisting}[language=bash,mathescape=false]
ng new tienda-app --package-manager=pnpm --routing --style=scss
ng config cli.packageManager pnpm \end{lstlisting}
							\end{itemize}
							...
							\begin{itemize}
								...
								\begin{itemize}
									...
									\item \emph{Características:} Amplio soporte, comandos estándar (\texttt{install}, \texttt{run}, \texttt{update}); integración inmediata con la mayoría de herramientas.
									\item \emph{Ejemplos:}
									\begin{lstlisting}[language=bash,mathescape=false]
npm ci                 # instalación limpia desde package-lock.json
npm run build          # ejecutar script "build"
npm install -g @angular/cli \end{lstlisting}
									\item \emph{Cuándo elegirlo:} opción segura y universal; ideal si el equipo de desarrollo ya está familiarizado con npm.
								\end{itemize}
								
								\item \textbf{pnpm}
								\begin{itemize}
									\item \emph{Definición:} Gestor con \emph{store} de contenido compartida y enlaces simbólicos; bloquea con \texttt{pnpm-lock.yaml}.
									\item \emph{Características:} Instalaciones más rápidas y eficientes en espacio; aislamiento más estricto por proyecto (evita dependencias fantasmas).
									\item \emph{Ejemplos:}
									\begin{lstlisting}[language=bash,mathescape=false]
corepack enable pnpm   # habilitar pnpm si viene con Node (Corepack)
pnpm install
pnpm run build
pnpm add -g @angular/cli \end{lstlisting}
										\item \emph{Cuándo elegirlo:} monorepos o equipos que buscan reproducibilidad y velocidad con \emph{store} compartida.
								\end{itemize}
								
								\item \textbf{yarn}
								\begin{itemize}
									\item \emph{Definición:} Alternativa popular; usa \texttt{yarn.lock}. Existen \emph{Yarn Classic} (v1) y \emph{Berry} (v2+).
									\item \emph{Características:} Caché agresiva, \emph{workspaces}, \emph{plug’n’play} (en Berry) para evitar \texttt{node\_modules} tradicionales.
									\item \emph{Ejemplos:}
									\begin{lstlisting}[language=bash,mathescape=false]
corepack enable yarn
yarn install
yarn build
yarn global add @angular/cli \end{lstlisting}
									\item \emph{Cuándo elegirlo:} proyectos que ya usan Yarn o que necesitan \emph{workspaces} y flujos específicos de Yarn.
								\end{itemize}
							
								\item \textbf{bun}
								\begin{itemize}
									\item \emph{Definición:} Runtime alternativo de JS/TS con su propio gestor (\texttt{bun add}); mantiene \texttt{bun.lockb}.
									\item \emph{Características:} Énfasis en rendimiento de instalación y ejecución; compatibilidad con gran parte del ecosistema Node.
									\item \emph{Ejemplos:}
									\begin{lstlisting}[language=bash,mathescape=false]
bun add -g @angular/cli
bun install
bun run build \end{lstlisting}
									\item \emph{Cuándo elegirlo:} si su equipo ya lo estandarizó y ha verificado compatibilidad con sus \emph{toolchains}. Para principiantes, siga npm o pnpm.
								\end{itemize}
							\end{itemize}
								...
								\item \textbf{Ejemplo completo (de cero a proyecto funcional con LTS y CLI).}
								\begin{itemize}
									\item \emph{Pasos:}
									\begin{lstlisting}[language=bash,mathescape=false]
...
ng build --configuration production \end{lstlisting}
									\item \emph{Resultado:} aplicación Angular lista para desarrollo local y con artefactos en \texttt{dist/} para desplegar.
								\end{itemize}
							\end{itemize}
...
# Windows: usar nvm-windows (comandos análogos). \end{lstlisting}
					...
					\begin{lstlisting}[language=bash,mathescape=false]
npm install -g @angular/cli
# o, con otros gestores (ver más abajo):
pnpm add -g @angular/cli
yarn global add @angular/cli
bun add -g @angular/cli \end{lstlisting}
					\item \emph{Verificar CLI:}
					\begin{lstlisting}[language=bash,mathescape=false]
						ng version
					\end{lstlisting}\begin{lstlisting}[mathescape=false]
node -v   # se recomienda LTS activo (p. ej., 20.x o 22.x)
npm -v\end{lstlisting}
		
					\item \textbf{Instalar Angular CLI} \cite{AngularCLI2025}:
...
bun install -g @angular/cli \end{lstlisting}
					...
					\item \textbf{Crear el proyecto} (con enrutamiento y estilos SCSS) \cite{AngularSetupLocal2025}:
					\begin{lstlisting}[mathescape=false]
ng new tienda-app --routing --style=scss
cd tienda-app
ng serve -o \end{lstlisting}
		
					\item \textbf{Extensiones útiles de editor}: Angular Language Service (autocompletado en plantillas), ESLint, Prettier \cite{AngularStyleGuide2025}.
					
					\item \textbf{Habilitar HttpClient y formularios} (si usa configuración \emph{standalone} moderna):
					\begin{lstlisting}[mathescape=false]
// main.ts (Angular v17+)
	...
	withXsrfConfiguration({ headerName: 'X-XSRF-TOKEN', cookieName: 'XSRF-TOKEN' })
		...
		\subsection{Arquitectura y Modelo de Programación}
			...
			\subsubsection{Gráfico de Arquitectura}
				La Figura \ref{fig:arquitectura-angular-spring} ilustra un flujo típico: componentes $\rightarrow$ servicios (HttpClient) $\rightarrow$ interceptores $\rightarrow$ API REST de Spring Boot $\rightarrow$ capa de persistencia.
				...
				\begin{itemize}
					\item \textbf{Proyecto Angular (estructura inicial de la aplicación).}
					\begin{itemize}
						\item \emph{Definición:} Conjunto de carpetas y archivos base (workspace) que incluye la aplicación, configuración de compilación, pruebas, \texttt{tsconfig}, \texttt{package.json}, y recursos. Esta estructura estandarizada permite a cualquier equipo iniciar un proyecto con convenciones claras y repetibles, lo cual reduce la curva de aprendizaje inicial y evita errores frecuentes en la configuración manual \cite{AngularDocs}.
						\item \emph{Características:} Soporte para enrutamiento opcional; preconfiguración de estilos (p.\,ej., SCSS); estructura basada en componentes; scripts de \emph{build}/\emph{test}. La presencia de estas características busca garantizar que, incluso sin experiencia previa, los desarrolladores puedan crear aplicaciones escalables y fáciles de mantener.
						\item \emph{Funcionalidades:} Punto de entrada \texttt{main.ts}, \texttt{AppComponent} inicial, configuración de \texttt{environment}, y arquitectura para DI y módulos/standalone. La inyección de dependencias (DI) facilita el desacoplamiento entre módulos y promueve la reutilización de código.
						\item \emph{Ejemplo de uso:} Iniciar el desarrollo a partir de \texttt{AppComponent}, añadir módulos de \texttt{HttpClient} e implementar la primera vista de inicio. Un proyecto típico evoluciona agregando gradualmente servicios, componentes y módulos a medida que crecen los requerimientos (ver Figura \ref{fig:estructura-angular}).
...
└── README.md\end{verbatim}
							\textcolor{Orange}{\textbf{\caption{Estructura típica de un proyecto Angular generado con la CLI. \label{fig:estructura-angular}}}}
						\end{figure}
					\end{itemize}
					
					\item \textbf{Servidor de desarrollo (\emph{dev server}) con recarga en caliente.}
					\begin{itemize}
						\item \emph{Definición:} Servidor local que sirve la aplicación en memoria y recompila al detectar cambios. Esto permite visualizar en tiempo real cada cambio realizado en la plantilla, estilos o lógica de un componente.
						\item \emph{Características:} El \emph{Hot Module Replacement} (HMR), traducido como “reemplazo en caliente de módulos”, evita tener que recargar toda la aplicación, manteniendo el estado de la misma mientras solo se actualizan las partes modificadas \cite{WebpackHMR}. Esto se complementa con los \emph{source maps}, que permiten depurar el código TypeScript viendo directamente las líneas que escribió el programador, y no el JavaScript transpilado.
						\item \emph{Funcionalidades:} Navegación inmediata para validar UI y flujos; integración con proxy para evitar CORS durante el desarrollo. Así, un principiante puede simular el comportamiento con un back-end sin tener que implementar mecanismos complejos de configuración.
						\item \emph{Ejemplo de uso:} Probar vistas y componentes mientras se edita código; configurar \texttt{proxy.conf.json} para redirigir \texttt{/api} al back-end. En este contexto, basta con modificar la interfaz de usuario en el editor y ver los resultados al instante en el navegador.
					\end{itemize}
					...
					\begin{itemize}
						...
						\item \emph{Características:} Minificación, eliminación de código muerto (\emph{tree-shaking}), división de código por rutas, comprobación de presupuestos de \emph{bundle}.
						\item \emph{Funcionalidades:} Produce HTML, JS y CSS optimizados; integra variables de \texttt{environment} para producción.
					...
					\begin{itemize}
						\item \emph{Definición:} Unidad de UI que encapsula plantilla, lógica y estilos para listar productos.
						\item \emph{Características:} Enfoque declarativo; enlace de datos (\emph{data binding}); \emph{change detection} optimizado; control de flujo moderno (\verb|@for|, \verb|@if|).
						\item \emph{Funcionalidades:} Renderiza una colección proveniente de un servicio; emite eventos (p.\,ej., seleccionar/editar).
						\item \emph{Ejemplo de uso:} Inyectar un servicio de productos, suscribirse a un \texttt{Observable<Producto[]>} y mostrar tarjetas o filas con \verb|@for (p of productos$ | async; track p.id)|.
					\end{itemize}
					
					\item \textbf{Servicio \texttt{core/productos}.}
					\begin{itemize}
						\item \emph{Definición:} Capa de acceso a datos y lógica compartida para productos (consumo de API). Es la parte encargada de conectar la aplicación con el back-end, aislando los detalles de comunicación del resto de la app.
						\item \emph{Características:} Inyectable vía DI; métodos tipados (\texttt{get/post/put/delete}); uso de RxJS (\texttt{Observable}). Un \emph{Observable} es un patrón de programación reactiva que representa flujos de datos a los que se pueden suscribir otros componentes \cite{RxJSObservableGuide}. En palabras simples, un observable es como una “fuente” de datos que va emitiendo valores, y los componentes escuchan esas emisiones para actualizarse automáticamente.
						\item \emph{Funcionalidades:} Encapsula URLs, parámetros y mapeo de respuestas; centraliza \emph{retry}, \emph{cache} y manejo de errores. Para un novato, esto significa que no necesita repetir en cada componente la lógica de conexión, sino que todo se encuentra en un lugar común.
						\item \emph{Ejemplo de uso:} \texttt{listar()}, \texttt{obtener(id)}, \texttt{crear(dto)}, \texttt{actualizar(id,dto)}, \texttt{eliminar(id)} consumidos por componentes y \emph{resolvers}. Es decir, un componente puede simplemente llamar a \texttt{productosService.listar()} y obtener la información sin preocuparse de los detalles técnicos de la petición HTTP.
					\end{itemize}
					
					\item \textbf{Guardia de rutas \texttt{auth/auth}.}
					\begin{itemize}
						\item \emph{Definición:} Mecanismo que decide si una ruta puede activarse (p.\,ej., \texttt{canActivate}).
						\item \emph{Características:} Integración con el enrutador; puede consultar estado de sesión/roles; redirecciones.
						\item \emph{Funcionalidades:} Protege áreas privadas; exige autenticación/autorización antes de cargar componentes o módulos perezosos.
						\item \emph{Ejemplo de uso:} Validar \texttt{isLoggedIn()} o un \texttt{role\_required}; en caso negativo, navegar a \texttt{/login}.
					\end{itemize}
...
@for (p of productos; track p.id) {
	<app-item [producto]="p" />
					...
					\item \textbf{Uso correcto de RxJS} (evitar fugas y \emph{work duplication}).\\
					\textit{Ejemplo}: cachear lecturas con \texttt{shareReplay(1)} y cancelar peticiones con \texttt{switchMap} \cite{RxJSObservableGuide,RxJSOperators}.
					\begin{lstlisting}[mathescape=false]
	...
	<React.StrictMode>
		<BrowserRouter>
			<App />
				...
				\begin{lstlisting}[language=typescript,mathescape=false]
// src/router.tsx
